	...
	<main class="flex flex-col space-y-4">
		...
		<div v-if="selectedOption === 'add'">
			...
			>
				...
				>
					<el-form-item label="出庫類型" :required="true">
						<el-select
							v-model="value1"
							placeholder="請選擇出庫類型"
							class="w-full"
						...
						>
							<el-option label="銷售換貨出庫" value="1"></el-option>
							<el-option label="採購退貨出庫" value="2"></el-option>
							<el-option label="報損出庫" value="3"></el-option>
							<el-option label="借出出庫" value="4"></el-option>
							<el-option label="借出直發出庫" value="5"></el-option>
							<el-option label="盤虧出庫" value="6"></el-option>
							<el-option label="調撥出庫" value="7"></el-option>
							<el-option label="銷售贈品出庫" value="8"></el-option>
							<el-option label="其他" value="9"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="出庫倉庫" :required="true">
						<el-select
							v-model="value2"
							placeholder="請選擇出庫倉庫"
							class="w-full"
					...
					<el-form-item label="其他出庫單編號" class="w-3/4">
						<el-input
							v-model="outboundID"
							disabled
						...
						<div class="w-full">
							...
							>
								...
								<el-table-column label="出庫倉位" width="150">
									<template #default="scope">
										<el-select
											v-model="scope.row.outboundWarehouse"
											placeholder="選擇倉位"
											class="w-full"
										>
											<el-option
				...
				<el-form
					...
					class="grid grid-cols-1 xl:grid-cols-3 gap-x-16 py-7 px-2 font-bold"
				>
					<el-form-item label="出庫確認" :required="true">
						<el-radio
							:model-value="radio1"
							:label="true"
...
// 表格數據
const tableOutboundData = reactive([
	{
		pdName: '暫無內容',
		pdCode: '暫無內容',
		brand: '暫無內容',
...
const deleteRow = (index) => {
	ElMessageBox.confirm('確定要刪除這筆資料嗎？', '提示', {
		...
		.then(() => {
			tableOutboundData.splice(index, 1);
		})
		.catch(() => {});
};
...
const detailsColumns = ref([
	{ prop: 'outboundType', label: '出庫類型', minWidth: 120 },
	{ prop: 'outboundWarehouse', label: '出庫倉庫', minWidth: 100 },
	{ prop: 'outboundID', label: '其他出庫單編號', minWidth: 150 },
	{ prop: 'deliveryAddress', label: '收貨地址', minWidth: 150 },
	{
		prop: 'pdOutboundDetails',
		label: '出庫產品明細',
		headerAlign: 'left',
		children: [
			{ prop: 'pdName', label: '產品名稱', minWidth: 180 },
...
const detailsData = ref([
	{
		...
		totalCost: 0,
		outboundTotalQty: 28,
		outboundTotalPrice: 36400,
		outboundTotalCost: 18830,
		outboundConfirm: '確認',
		outboundImg: '',
		outboundTime: '220 02:00',
		warehouseStaff: '阿依',
	...
	},
	{
		...
		totalCost: 183,
		outboundTotalQty: 20,
		outboundTotalPrice: 28400,
		outboundTotalCost: 14750,
		outboundConfirm: '確認',
		outboundImg: '',
		outboundTime: '2024/03/31 02:00',
		warehouseStaff: '阿二',
		warehouseDepartment: '市場部',
		submitter: '阿二',
		submitTime: '2024/01/15 15:30',
...
// 快捷表格欄位
const cvColumns = ref([
	{ prop: 'outboundQty', label: '本次出庫數量' },
	{ prop: 'outboundWarehouse', label: '出庫倉位' },
...
const confirmSelection = () => {
	// 當點擊確定時更新 tableOutboundData
	tableOutboundData.length = 0; // 清空舊數據
	selectedData.value.forEach((item) => {
		...
		tableOutboundData.push(updatedRow);
	});
	openData.value = false; // 關閉對話框
};
// 搜尋
const outboundFilterData = computed(() =>
	outboundData.value.filter(
		(data) =>
			!searchText.value ||
