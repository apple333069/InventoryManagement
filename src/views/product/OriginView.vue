<template>
	<main class="flex flex-col space-y-4">
			...
			<div class="p-8 bg-white rounded-md shadow">
				...
				>
					<div class="xl:col-span-4">
						<el-form-item label="產地編碼" :required="true">
							<el-input
								class="xl:max-w-lg"
								disabled
					...
					<!-- 左側欄位 -->
					<div>
						<el-form-item label="產地名稱" :required="true">
							<el-input placeholder="請輸入產地名稱" class="w-full"></el-input>
						</el-form-item>

						<el-form-item label="產地地址" :required="true">
							<el-cascader
								:options="options"
								class="w-full"
								placeholder="請選擇位址"
						...
						<el-form-item label="產地類型" :required="true">
							<el-select
								v-model="value2"
								placeholder="請選擇"
								class="w-full"
						...
						<el-form-item label="國家代碼">
							<el-input placeholder="請輸入國家代碼" class="w-full"></el-input>
						</el-form-item>
					</div>

					<!-- 右側欄位 -->
					<div class="xl:col-start-3">
						<el-form-item label="供應商" :required="true">
							<el-select
								v-model="value2"
								placeholder="請選擇"
								class="w-full"
								...
								:no-data-text="'沒有可選擇的數據'"
							>
								<el-option label="供應商1" value="1"></el-option>
								<el-option label="供應商2" value="2"></el-option>
								<el-option label="供應商3" value="3"></el-option>
							</el-select>
						</el-form-item>

						<el-form-item label="是否啟用" :required="true">
						...
						<el-form-item label="建立日期" class="w-3/4">
							<el-date-picker
								v-model="createdAt"
								type="date"
								style="width: 100%"
		...
		<div v-else-if="selectedOption === 'manage'">
			<el-tabs
				v-model="activeManage"
				class="p-2 bg-white rounded-md shadow min-h-screen overflow-auto"
			>
				<el-tab-pane name="1">
					<template #label>
		...
		<div v-else-if="selectedOption === 'restricted'">
			<div class="p-2 bg-white rounded-md shadow min-h-screen overflow-auto">
				<ManageTable :tableData="detailsData" :columns="detailsColumns">
					<template #pdPermission="{ row }">
						<el-tag
							v-for="(permission, index) in row.pdPermission"
...
onMounted(() => {
	const now = new Date();
	createdAt.value = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(
		2,
		'0'
